...
void w25q64Init(SPI_HandleTypeDef *hspi, GPIO_TypeDef *gpio, uint16_t pin) {
    if (w25q64_hspi == NULL || w25q64_gpio == NULL) return;
    w25q64_hspi = hspi;
    w25q64_gpio = gpio;
    w25q64_pin = pin;
...
uint32_t w25q64ReadUID(void) {
    ...
    spiSendRecvData(w25q64_hspi, w25q64_gpio, w25q64_pin, buff, 1 + 4 + 8);
    uint32_t id = 0;
...
uint8_t w25q64EraseSector(uint32_t address) {
    ...
    if (w25q64WaitWriteEnable(W25Q64_WAIT_TIME) > 0) {
        ...
        // 发送 1 字节命令, 接收 3 字节地址
        if (spiSendRecvData(w25q64_hspi, w25q64_gpio, w25q64_pin, buff, 1 + 3) == HAL_OK) ret = RET_DONE;
    }
    return ret;
}
...
uint8_t w25q64EraseChip(void) {
    ...
    if (w25q64WaitWriteEnable(W25Q64_WAIT_TIME) > 0) {
        ...
        // 发送 1 字节命令
        if (spiSendRecvData(w25q64_hspi, w25q64_gpio, w25q64_pin, buff, 1) == HAL_OK) ret = RET_DONE;
    }
    return ret;
}
...
BaseType_t w25q64Read(uint32_t address, uint8_t *data, uint32_t length) {
    ...
    while (length > 0) {
        ...
        if (w25q64WaitIdle(W25Q64_WAIT_TIME) > 0) {
            ...
            result = spiSendRecvData(w25q64_hspi, w25q64_gpio, w25q64_pin, buff, read_len + 4);
            // 拷贝数据
...
BaseType_t w25q64WritePage(uint32_t address, uint8_t *data, uint32_t length) {
    ...
    if (w25q64WaitWriteEnable(W25Q64_WAIT_TIME) > 0) {
        ...
        result = spiSendRecvData(w25q64_hspi, w25q64_gpio, w25q64_pin, buff, write_len + 4);
    }
    vPortFree(buff);
    return result;
}
